...
//// this work so well!!
//// https://github.com/naudio/NAudio/blob/master/NAudio/Midi/MidiMessage.cs
...
namespace Poubub.HumanInterface
{
    ...
    public class MidiMessage
    {
        ...
        public static MidiEvent StartNote(int note, int volume, int channel)
        {
            channel++;
            ValidateNoteParameters(note, volume, channel);
            return new MidiEvent(Convert.ToByte(MidiEvent.NoteOn + channel - 1), Convert.ToByte(note), Convert.ToByte(volume), null);
           // return new MidiMessage((int)MidiEvent.NoteOn + channel - 1, note, volume);
        }
        ...
        /// <returns>A new MidiMessage object</returns>
        public static MidiEvent StopNote(int note, int volume, int channel)
        {
            channel++;
            ValidateNoteParameters(note, volume, channel);
            return new MidiEvent(Convert.ToByte(MidiEvent.NoteOff + channel - 1), Convert.ToByte(note), Convert.ToByte(volume), null);
           // return new MidiMessage((int)MidiEvent.NoteOff + channel - 1, note, volume);
        }
        ...
        public static MidiEvent ChangePatch(int patch, int channel)
        {
            channel++;
            ValidateChannel(channel);
            return new MidiEvent(Convert.ToByte(MidiEvent.Program + channel - 1), Convert.ToByte(patch), Convert.ToByte(channel), null);
           // return new MidiMessage((int)MidiEvent.Program + channel - 1, patch, 0);
        }
        ...
        /// <param name="channel">The MIDI channel number (1-16)</param>
        /// <returns>A new MidiMessageObject</returns>
        public static MidiEvent ChangeControl(int controller, int value, int channel)
        {
            channel++;
            ValidateChannel(channel);
            return new MidiEvent(Convert.ToByte(MidiEvent.CC + channel - 1), Convert.ToByte(controller), Convert.ToByte(value), null);
           // return new MidiMessage((int)MidiEvent.CC + channel - 1, controller, value);
        }
        ...
        public byte[] RawData
        {
            get
            {
                //return new byte[] { Convert.ToByte(rawData) };
                return new byte[] {
                    Convert.ToByte(rawData & 0xff),
                    Convert.ToByte((rawData >> 8) & 0xff),
                    Convert.ToByte(  (rawData >> 16) & 0xff)
                };
            }
        }
    }
